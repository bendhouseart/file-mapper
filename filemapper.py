...
def get_parser():
    ...
    parser.add_argument(
        "jsonpath", nargs=1, help="""Absolute path to the required JSON."""
    )
...
def do_action(src, dest, action, overwrite=False, testdebug=False, relsym=False):
    if relsym:
        ...
        rel_src_from_dest = os.sep.join(
            [".." for _ in dest_from_common.split(os.sep)[:-1]] + [src_from_common]
        )
        rel_dest_from_src = os.sep.join(
            [".." for _ in src_from_common.split(os.sep)[:-1]] + [dest_from_common]
        )
...
def parse_template(template_str, testdebug=False, verbose=False):
    ...
    if equal_count == (comma_count + 1):
        ...
        print(
            "There is not an even split of commas and equal signs within this template input: "
            + template_str
        )
        print(
            "The --template argument takes its argument in the format: 'A=B,C=D,E=F'"
            + template_str
        )
        print("Exiting.")
        sys.exit()
...
def parse_data(
    ...
    for key in non_specials_list:
        # If sourcepath exists the append it to the destination value and overwrites
        if sourcepath is not None:
            if verbose:
                print("A sourcepath argument exists")
            source = os.path.join(sourcepath, key)
            if "SOURCE" in data:
                if verbose:
                    print(
        ...
        if destpath is not None:
            ...
            destination = os.path.join(destpath, data[key])
            if "DESTINATION" in data:
                if verbose:
                    print(
                        "Optional DESTINATION argument: "
                        + destpath
                        + " overrules destination: "
        ...
        if os.path.exists(source) and os.path.isfile(source):
            ...
            if os.path.isfile(destination):
                ...
                do_action(
                    source, destination, action, testdebug=testdebug, relsym=relsym
                )
                if sidecars:
                    json_src, json_dest = map_sidecars(source, destination)
                    do_action(
                        json_src, json_dest, action, testdebug=testdebug, relsym=relsym
                    )
            else:
                os.makedirs(dirname)
                if verbose:
...
def json_validator(filename, skip_errors=False, **kwargs):
...
def process_json_file(
    ...
    """
    Main function to process a JSON file with file mapping operations.
    This function can be called directly without using argparse.
    """
...
def main():
    ...
    sourcepath = args.sourcepath[0] if args.sourcepath else None
    destpath = args.destpath[0] if args.destpath else None
